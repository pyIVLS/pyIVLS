...
import pluggy
from fastPulse_GUI import fastPulse_GUI


class pyIVLS_fastPulse_plugin:
    """Hooks for fastPulse plugin"""
    ...
    def get_setup_interface(self, plugin_data) -> dict:
        ...
        if hasattr(self.fastPulse, "set_dependencies"):
            self.fastPulse.set_dependencies(self.dependencies)
        if "function_dict" in plugin_data[self.name]:
            self.fastPulse.function_dict = plugin_data[self.name]["function_dict"]
        self.fastPulse._initGUI(plugin_data[self.name]["settings"])
        return {self.name: self.fastPulse.settingsWidget}
    ...
    def get_functions(self, args=None):
        ...
        if args is None or args.get("function") == self.function:
            return {self.name: self.fastPulse._get_public_methods()}
    ...
    def get_log(self, args=None):
        ...
        """
        if args is None or args.get("function") == self.function:
            return {self.name: self.fastPulse.logger.logger_signal}
    ...
    def get_info(self, args=None):
        ...
        if args is None or args.get("function") == self.function:
            return {self.name: self.fastPulse.logger.info_popup_signal}
