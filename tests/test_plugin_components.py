...
class TestSMUHelper:
    """Test the SMUHelper class for SMU initialization."""

    def setup_method(self):
        """Set up test fixtures."""
        self.smu_helper = SMUHelper("TestPlugin")

    def test_init(self):
        """Test SMUHelper initialization."""
        assert self.smu_helper.plugin_name == "TestPlugin"

    def test_smu_init_voltage_mode_single_channel(self):
        """Test SMU initialization for voltage mode, single channel."""
        plugin_settings = {
            "channel": "smua",
            "drainchannel": "smub",
            "inject": "voltage",
            ...
            "sourcesensemode": "4 wire",
            "drainnplc": 0.02,
            "draindelaymode": "manual",
            "draindelay": 300,
            "drainlimit": 0.05,
            "drainsensemode": "2 wire",
        }

        smu_settings = {"sourcehighc": True, "drainhighc": False}

        mock_smu_functions = {
            "smu_init": Mock(return_value=False)  # Success
        }

        status, state = self.smu_helper.smu_init(plugin_settings, smu_settings, mock_smu_functions)

        # Check that smu_init was called
        mock_smu_functions["smu_init"].assert_called_once()

        # Get the settings passed to smu_init
        call_args = mock_smu_functions["smu_init"].call_args[0][0]

        # Verify the settings
        assert call_args["pulse"] is False
        assert call_args["source"] == "smua"
        assert call_args["drain"] == "smub"
        assert call_args["type"] == "v"  # Voltage mode
        assert call_args["single_ch"] is True
        assert call_args["sourcenplc"] == 0.01
        assert call_args["delay"] is True  # Auto delay mode
        assert call_args["delayduration"] == 500
        assert call_args["limit"] == 0.1
        assert call_args["sourcehighc"] is True
        assert call_args["sourcesense"] is True  # 4 wire
        assert call_args["drainsense"] is False  # 2 wire

        # Check return value for success (returns a tuple with status and message dict)
        assert status == 0  # Success status
        assert state["Error message"] == "OK"

    def test_smu_init_current_mode_dual_channel(self):
        """Test SMU initialization for current mode, dual channel."""
        plugin_settings = {
            "channel": "smua",
            "drainchannel": "smub",
            "inject": "current",
            ...
            "draindelay": 300,
            "drainlimit": 5.0,
            "drainsensemode": "4 wire",
        }

        smu_settings = {"sourcehighc": False, "drainhighc": True}
        ...
        # Verify current mode specific settings
        assert call_args["type"] == "i"  # Current mode
        assert call_args["single_ch"] is False
        assert call_args["draindelay"] is True  # Auto drain delay
        assert call_args["drainhighc"] is True
        assert call_args["sourcesense"] is False  # 2 wire
        assert call_args["drainsense"] is True  # 4 wire

        # Check return value for success (returns a tuple with status and message dict)
        assert status == 0
        assert state["Error message"] == "OK"

    def test_smu_init_failure(self):
        """Test SMU initialization failure."""
        plugin_settings = {
            "channel": "smua",
            "drainchannel": "smub",
            "inject": "voltage",
        ...
        smu_settings = {"sourcehighc": False, "drainhighc": False}

        mock_smu_functions = {
            "smu_init": Mock(return_value=True)  # Failure (non-zero return)
        }

        result = self.smu_helper.smu_init(plugin_settings, smu_settings, mock_smu_functions)
