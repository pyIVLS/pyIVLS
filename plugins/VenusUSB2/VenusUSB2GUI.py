...
class VenusUSB2GUI(QObject):
    ...
    def _parseSaveData(self) -> "status":
        ...
        if not os.path.isdir(self.settings["address"] + os.sep):
            self.log_message.emit(datetime.now().strftime("%H:%M:%S.%f") + " : VenusUSB2 plugin : address string should point to a valid directory")
            return [
                1,
        ...
        if not is_valid_filename(self.settings["filename"]):
            self.log_message.emit(datetime.now().strftime("%H:%M:%S.%f") + " : VenusUSB2 plugin : filename is not valid")
            self.info_message.emit("VenusUSB2 plugin : filename is not valid")
