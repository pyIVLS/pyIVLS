...
########Functions
    ...
    def __init__(self):
        ...
        self.dependency = {
        "sweep": ["parse_settings_widget", "sweepImplementation", "smu_connect", "sweepImplementation", "set_running"],  
        }
...
###############GUI react to change
    ...
    def _set_running(self, status):
        ...
        self.function_dict["sweep"]["set_running"](not status)                
    ...
    def _getPublicFunctions(self,function_dict):
        ...
        for dependency_plugin in list(self.dependency.keys()):
                if not dependency_plugin in function_dict:
                        self.log_message.emit(datetime.now().strftime("%H:%M:%S.%f") + f" : sweep plugin : Functions for dependency plugin '{dependency_plugin}' not found")
                ...
                for dependency_function in self.dependency[dependency_plugin]:
                        if not dependency_function in function_dict[dependency_plugin]:
                                self.log_message.emit(datetime.now().strftime("%H:%M:%S.%f") + f" : sweep plugin : Function '{dependency_function}' for dependency plugin '{dependency_plugin}' not found")
...
########plugin actions   
    ...
    def _runAction(self):
        ...
        if self._parse_settings_widget():
                #### enable the interface
                self._set_running(False)
                return 1
        [status, self.sweep_settings] = self.function_dict["sweep"]["parse_settings_widget"]()
        if status:
                if status == 2:
        ...
        [status, message] = self.function_dict["sweep"]["smu_connect"]()
        if status:
...
########sequence implementation  
    def _sweepImplementation(self, self.settings) :
    	try:
                [status, message] = self.function_dict["sweep"]["sweepImplementation"]
                if status:
                        ...
                        try:
                                self.function_dict["sweep"]["smu_abort"]
                                self.function_dict["sweep"]["smu_outputOFF"]
                                self.function_dict["sweep"]["smu_disconnect"]
                        except:
